...
{
    public class RFIDChestNoMappingRepository
    {
        private readonly DatabaseContext _dbContext;

        public RFIDChestNoMappingRepository(DatabaseContext dbContext)
        {
        ...
        public async Task<IActionResult>RFIDChestNoMapping(RFIDChestNoMappingDto model)
        {
            ...
            {
                var parameter = SetAppeal(model);
                try
                {
                    var sqlConnection = (Microsoft.Data.SqlClient.SqlConnection)connection;
                    await sqlConnection.OpenAsync();
                    var queryResult = await connection.QueryMultipleAsync("Proc_RFIDChestNoMapping", parameter, commandType: CommandType.StoredProcedure);
        ...
        public async Task<IActionResult> GetRFID(RFIDChestNoMappingDto model)
        {
            using (var connection = _dbContext.CreateConnection())
            {
                ...
                try
                {
                    var sqlConnection = (Microsoft.Data.SqlClient.SqlConnection)connection;
                    await sqlConnection.OpenAsync();
                    var queryResult = await connection.QueryMultipleAsync("Proc_RFIDRunningLog", parameter, commandType: CommandType.StoredProcedure);
                    var Model = queryResult.ReadSingleOrDefault<Object>();
                    var outcome = queryResult.ReadSingleOrDefault<Outcome>();
                    var outcomeId = outcome?.OutcomeId ?? 0;
                    var outcomeDetail = outcome?.OutcomeDetail ?? string.Empty;
        ...
        public DynamicParameters SetAppeal(RFIDChestNoMappingDto user)
        {
